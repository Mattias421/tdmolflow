...
import dnnlib
from training.egnn_utils import Jump_EGNN_QM9


# -------------------------------------------------------------
# specific changes to an object if the new source code has changed
def modify_network_pkl(net):

    try:
        jump_egnn_qm9 = net.model.egnn_net
    except:
        return

    if not isinstance(jump_egnn_qm9, Jump_EGNN_QM9):
        return

    try:
        tmp = jump_egnn_qm9.return_last_layer
    except:
        net.model.egnn_net.return_last_layer = False
...
class InfiniteSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        if len(self.dataset) == 10**12:
            yield from self.count_to_inf()  # interpret as infinite, do not shuffle
