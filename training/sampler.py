...
class StackedRandomGenerator:
    ...
    def rand(self, size, **kwargs):
        ...
        return torch.stack(
            [
                torch.rand(size[1:], generator=gen, **kwargs)
                for gen in self.generators
            ]
        )
    ...
    def poisson(self, input):
        ...
        return torch.stack(
            [
                torch.poisson(input[i], generator=gen)
    ...
    def multinomial(self, input, **kwargs):
        ...
        return torch.stack(
            [
                torch.multinomial(input[i, ...], generator=gen, **kwargs)
                for i, gen in enumerate(self.generators)
            ]
        )
...
class JumpSampler:
    ...
    def get_score(self, state_st_batch, net, loss, ts, dataset_obj, rnd):
        if not self.do_conditioning:
            ...
            x0_pred = loss.noise_schedule.predict_x0_from_xt(
                state_st_batch.get_flat_lats(), D_xt, ts
            )

            condition_st_batch, condition_mask = dataset_obj.condition_state(
                state_st_batch,
                self.condition_type,
                self.condition_sweep_idx,
                self.condition_sweep_path,
            )
            condition_dims = condition_st_batch.get_dims()

            x0_pred_of_cond = condition_mask * x0_pred
            x0_pred_of_cond_st_batch = StructuredDataBatch.create_copy(
                state_st_batch
            )
            ...
            x0_pred_of_cond_st_batch.gs.adjust_st_batch(
                x0_pred_of_cond_st_batch
            )
            x0_pred_of_cond = x0_pred_of_cond_st_batch.get_flat_lats()

            l2_error = torch.sum(
                condition_mask
                * (x0_pred_of_cond - condition_st_batch.get_flat_lats()) ** 2,
                dim=1,
            )  # (B,)

            unit_st_batch = StructuredDataBatch.create_copy(state_st_batch)
            unit_st_batch.set_flat_lats(
                torch.ones_like(unit_st_batch.get_flat_lats())
            )
            alpha_t = loss.noise_schedule.get_p0t_stats(unit_st_batch, ts)[0][
                :, 0
            ]  # (B,)
            ...
            x0_pred_adjusted = (
                condition_mask * condition_st_batch.get_flat_lats()
                + (1 - condition_mask) * (x0_pred + guidance_grad)
            )

            x0_with_condition_st_batch = StructuredDataBatch.create_copy(
                state_st_batch
            )
            ...
            x0_with_condition_st_batch.gs.adjust_st_batch(
                x0_with_condition_st_batch
            )
            x0_with_condition = x0_with_condition_st_batch.get_flat_lats()
            eps_pred = loss.noise_schedule.predict_eps_from_x0_xt(
                state_st_batch, x0_with_condition, ts
            )
    ...
    def sample(
        self, net, in_st_batch, loss, rnd, known_dims=None, dataset_obj=None
    ):
        print("---------- jump sampler -------------")
        ...
        while True:
            ...
            # corrector steps
            if (
                ts.min() < self.corrector_start_time
                and ts.max() > self.corrector_finish_time
            ):
