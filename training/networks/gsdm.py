...
class ResnetBlock(nn.Module):
    def __init__(
        self, *, channels, dropout, temb_channels=512, node_emb_channels=None
    ):
        super().__init__()
        make_linear = lambda a, b: nn.Conv1d(
            a, b, kernel_size=1, stride=1, padding=0
        )
...
class AttnBlock(nn.Module):
    ...
    def sparse_forward(self, x, sparse_attention_mask_and_indices):
        ...
        valid_indices_mask, attendable_indices = (
            sparse_attention_mask_and_indices
        )
        b_mask, nq, max_attendable_keys = valid_indices_mask.shape
        assert b_mask == b or b_mask == 1
        attendable_indices = attendable_indices.view(
            1, 1, nq, max_attendable_keys
        ...
        def get_keys_or_values(t, indices):
            *batch_shape, nd, nv = t.shape
            ...
            index = indices.view(
                *batch_shape, nv, max_attendable_keys, 1
            ).expand(-1, -1, -1, -1, c // heads)
            return t.gather(dim=2, index=index)

        attended_keys = get_keys_or_values(
            k, indices=attendable_indices
        )  # b x heads x h*w x max_attendable_keys x c
    ...
    def forward(
        self,
        x,
        sparsity_matrix=None,
        sparse_attention_mask_and_indices=None,
        return_w=False,
    ):
