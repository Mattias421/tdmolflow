...
from .base import GraphicalStructureBase, StructuredDatasetBase
    ...
    with open(gdb9_txt_thermo) as f:
        for line in f:
            ...
            for therm_target, split_therm in zip(therm_targets, split[1:]):
                therm_energy[therm_target][id2charge[split[0]]] = float(
                    split_therm
                )
...
def process_xyz_files(
    ...
    if stack:
        molecules = {
            key: (
                pad_sequence(val, batch_first=True)
                if val[0].dim() > 0
...
def prepare_dataset(
    ...
    split_names = (
        splits.keys() if splits is not None else ["train", "valid", "test"]
    )

    # Assume one data file for each split
    datafiles = {
        split: os.path.join(*(dataset_dir + [split + ".npz"]))
        for split in split_names
    }

    # Check datafiles exist
    datafiles_checks = [
        os.path.exists(datafile) for datafile in datafiles.values()
    ]
...
def _get_species(datasets, ignore_check=False):
    ...
    if not all(
        [
            split.tolist() == all_species.tolist()
            for split in split_species.values()
        ]
    ):
...
def initialize_datasets(
    ...
    for split, datafile in datafiles.items():
        with np.load(datafile) as f:
            datasets[split] = {
                key: torch.from_numpy(val) for key, val in f.items()
            }
    ...
    if dataset != "qm9":
        ...
        if dataset == "qm9_second_half":
            ...
            sliced_perm = fixed_perm[
                0 : len(datasets["train"]["num_atoms"]) // 2
            ]
        else:
    ...
    assert (
        len(
            set(
                tuple(data.included_species.tolist())
...
def batch_stack(props):
    ...
    if not torch.is_tensor(props[0]):
        ...
        return torch.nn.utils.rnn.pad_sequence(
            props, batch_first=True, padding_value=0
        )
...
def get_dataset_info(dataset_name, remove_h):
    if dataset_name == "qm9":
        ...
        if not remove_h:
            ...
            raise Exception(
                "Missing config for %s without hydrogens" % dataset_name
            )
...
def compute_mean_mad(dataloaders, properties, dataset_name):
    if dataset_name == "qm9":
        return compute_mean_mad_from_dataloader(
            dataloaders["train"], properties
        )
    elif dataset_name == "qm9_second_half" or dataset_name == "qm9_second_half":
        return compute_mean_mad_from_dataloader(
            dataloaders["valid"], properties
        )
    else:
        raise Exception("Wrong dataset name")
...
def check_stability(positions, atom_type, dataset_info, debug=False):
    ...
    for i in range(len(x)):
        for j in range(i + 1, len(x)):
            ...
            atom1, atom2 = (
                atom_decoder[atom_type[i]],
                atom_decoder[atom_type[j]],
            )
...
def _arg_to_bool(arg):
    ...
    if type(arg) is bool:
        ...
        raise ValueError(
            "Input must be boolean or string! {}".format(type(arg))
        )
...
class QM9Dataset(StructuredDatasetBase):
    ...
    def __init__(
        ...
        self.dataloaders = {
            "train": DataLoader(datasets["train"], batch_size=32)
        }
    ...
    def __getitem__(self, index, will_augment=False):
        ...
        assert atom_types.shape == (
            self.graphical_structure.max_problem_dim,
            self.num_atom_types,
        )
        ...
        if self.condition_on_alpha:
            alpha = (
                self.datasets["train"].data["alpha"][index, ...]
                - self.datasets["train"].stats["alpha"][0]
            ) / self.datasets["train"].stats["alpha"][1]
            alpha = alpha.view(1)
        else:
            alpha = torch.zeros((1,))
    ...
    def log_batch(self, in_st_batch, out_st_batch, wandb_log=True):
        ...
        validity_dict, rdkit_tuple = analyze_stability_for_molecules(
            molecules, self.dataset_info, dataloaders=self.dataloaders
        )
    ...
    def condition_state(
        ...
        if (self.condition_st_batch is None) or (
            self.condition_st_batch.B != batch
        ):
            if condition_type == "sweep":
                ...
                assert condition_data.shape == (
                    10,
                    6,
                )  # first 4 numbers are C N O F next two can ignore
                condition_row = torch.from_numpy(
                    condition_data[condition_sweep_idx, 0:4]
                ).int()
                ...
                for i in range(4):
                    one_hot_cond[
                        condition_row[0:i]
                        .sum() : condition_row[0 : i + 1]
                        .sum(),
                        i + 1,
                    ] = 1  # i+1 because we're not doing H

                self.condition_st_batch = StructuredDataBatch.create_copy(
                    state_st_batch
                )
                n_nodes = self.condition_st_batch.gs.max_problem_dim
                condition_flat_lats = torch.zeros_like(
                    self.condition_st_batch.get_flat_lats()
                )  # (B, 261)
                condition_flat_lats[
                    :, 3 * n_nodes : 3 * n_nodes + 5 * total_cond_dim
                ] = (one_hot_cond.float().flatten() / self.norm_values[1])
                self.condition_st_batch.set_flat_lats(condition_flat_lats)
                self.condition_st_batch.set_dims(
                    total_cond_dim * torch.ones((batch,))
                )
                self.condition_st_batch.delete_dims(
                    new_dims=total_cond_dim * torch.ones((batch,))
                )
                self.condition_mask = torch.zeros_like(
                    self.condition_st_batch.get_flat_lats()
                )  # (B, 261)
                self.condition_mask[
                    :, 3 * n_nodes : 3 * n_nodes + 5 * total_cond_dim
                ] = torch.ones((5 * total_cond_dim,)).to(device)
            else:
                raise ValueError(
                    "Condition type not recognized: ", condition_type
                )
...
class QM9GraphicalStructure(GraphicalStructureBase):
    ...
    def remove_problem_dims(self, data, new_dims):
        ...
        pos_mask = (
            torch.arange(pos.shape[1], device=device)
            .view(1, -1, 1)
            .repeat(pos.shape[0], 1, pos.shape[2])
        )
        pos_mask = pos_mask < new_dims_dev.view(-1, 1, 1)
        pos = pos * pos_mask

        atom_type_mask = (
            torch.arange(atom_type.shape[1], device=device)
            .view(1, -1, 1)
            .repeat(atom_type.shape[0], 1, atom_type.shape[2])
        )
        atom_type_mask = atom_type_mask < new_dims_dev.view(-1, 1, 1)
        atom_type = atom_type * atom_type_mask

        charge_mask = (
            torch.arange(charge.shape[1], device=device)
            .view(1, -1)
            .repeat(charge.shape[0], 1)
        )
    ...
    def adjust_st_batch(self, st_batch):
        ...
        nan_batches = (
            torch.isnan(st_batch.get_flat_lats()).any(dim=1).long().view(B, 1)
        )
        ...
        node_mask[dims == 0, ...] = torch.ones((B, n_nodes, 1), device=device)[
            dims == 0, ...
        ].long()
        ...
        ), f"Error {masked_max_abs_value} too high"
    ...
    def get_auto_target(self, st_batch, adjust_val):
        ...
        atom_mask = torch.arange(n_nodes).view(
            1, -1
        ) < st_batch.get_dims().view(
            -1, 1
        )  # (B, n_nodes)
        assert atom_mask.shape == (B, n_nodes)
        atom_mask = atom_mask.long().to(device)
    ...
    def get_nearest_atom(self, st_batch, delxt_st_batch):
        ...
        missing_atom_pos = x_full[
            torch.arange(B, device=device).long(), (full_dims - 1).long(), :
        ]  # (B, 3)

        distances_to_missing = torch.sum(
            (x_del - missing_atom_pos.unsqueeze(1)) ** 2, dim=2
        )  # (B, n_nodes)

        atom_mask = torch.arange(st_batch.gs.max_problem_dim).view(
            1, -1
        ) < delxt_st_batch.get_dims().view(
            -1, 1
        )  # (B, n_nodes)
        atom_mask = atom_mask.to(device).long()

        distances_to_missing = (
            atom_mask * distances_to_missing + (1 - atom_mask) * 1e3
        )
...
def compute_qm9_smiles(dataset_name, remove_h, dataloaders):
    ...
    for i, data in enumerate(dataloaders["train"]):
        ...
        mol = build_molecule(
            torch.tensor(positions), torch.tensor(atom_type), dataset_info
        )
        mol = mol2smiles(mol)
        if mol is not None:
