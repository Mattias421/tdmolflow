    ...
    def __call__(self, net, st_batch):
        ...
        if self.nearest_atom_pred:
            D_xt, rate_xt, dummy_mean_std, x0_dim_logits, _ = net(
                ...
                predict=to_predict,
                nearest_atom=torch.zeros((B,), device=device).long(),
            )
        else:
            D_xt, rate_xt, dummy_mean_std, x0_dim_logits = net(
                st_batch,
                ts=ts.to(device),
                forward_rate=self.forward_rate,
                predict=to_predict,
        ...
        ce_loss = F.cross_entropy(
            x0_dim_logits, x0_dims.to(device) - 1, reduction="none"
        )
        assert ce_loss.shape == (B,)

        D_xt_mask = st_batch.get_mask(
            B, include_obs=False, include_onehot_channels=True
        ).to(st_batch.get_device())
        ...
        if self.nearest_atom_pred:
            nearest_atom = st_batch.gs.get_nearest_atom(
                st_batch, delxt_st_batch
            )
        ...
        rate_loss = (dims_xt < st_batch.gs.max_problem_dim) * rate_xt[:, 0] - (
            dims_xt > 1
        ) * f_rate_vs_t * torch.log(rate_delxt[:, 0] + 1e-12)
        assert rate_loss.shape == (B,)

        final_dim_mask = st_batch.get_next_dim_deleted_mask(
        ...
        if sum(dims_xt > 1) > 0:
            ...
            auto_loss = (
                ...
                * torch.sum(
                    ...
                    * (
                        -torch.log(std)
                        - 0.5 * (1 / (std**2)) * (auto_target - mean) ** 2
                    ),
                    dim=1,
                )
            )
        ...
        if self.nearest_atom_pred:
            nearest_atom_loss = (dims_xt > 1) * F.cross_entropy(
                near_atom_logits, nearest_atom, reduction="none"
            )
        ...
        loss = (
            ...
            + self.x0_logit_ce_loss_weight
            * (1 / x.shape[1])
            * ce_loss.view(B, 1)
            + self.nearest_atom_loss_weight
            * (1 / x.shape[1])
            * nearest_atom_loss.view(B, 1)
        )
...
JumpLossFinalDim_to_kwargs = {
    JumpLossFinalDim: set(
        [
            (
                "rate_function_name",
                "click.Choice(['const', 'quartic', 'step', 'cosine', 'dimsteprate', 'pendulum', 'triangle', 'twostep'])",
                "const",
            ),
            ("min_t", "float", 0.001),
